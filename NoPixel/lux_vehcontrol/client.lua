...
AddEventHandler("lvc_TogIndicState_c", function(sender, newstate)
	if not DoesPlayerExist(sender) then return end
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if not IsPauseMenuActive() then
							...
							-- TOG DFLT SRN LIGHTS
							if IsDisabledControlJustReleased(0, Controlkey["vehicleSlights"][1]) and not focusTaken then 
							...
							-- TOG LX SIREN
							elseif IsDisabledControlJustReleased(0, Controlkey["vehicleSsound"][1]) and not focusTaken then
							...
							if state_lxsiren[veh] > 0 then
								if IsDisabledControlJustReleased(0, Controlkey["vehicleSnavigate"][1]) and not focusTaken then
									if IsVehicleSirenOn(veh) then
										local cstate = state_lxsiren[veh]
										local nstate = 1
										PlaySoundFrontend(-1, "NAV_LEFT_RIGHT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1) -- on
							...
							if state_lxsiren[veh] < 1 then
								if IsDisabledControlPressed(0, Controlkey["vehicleSnavigate"][1]) and not focusTaken then -- nfi same as above
									actv_manu = true
								else
									actv_manu = false
								end
							...
							-- HORN
							if IsDisabledControlPressed(0, 86) and not focusTaken then
								actv_horn = true
							else
								actv_horn = false
							end
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsDisabledControlJustReleased(0, 84) and not focusTaken then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer			
							-- IND R
							elseif IsDisabledControlJustReleased(0, 83) and not focusTaken then -- INPUT_VEH_NEXT_RADIO_TRACK
								local cstate = state_indic[veh]
