...
_Queue.JoinCbs = {}
_Queue.TempPriority = {}
_Queue.isServerActive = 1
_Queue.isServerShuttingDown = 0
_Queue.MaxConnecting = 8
...
local joinDelay = GetGameTimer() + 90000
...
function Queue:GetServerActive()
    if _Queue.isServerActive == 1
    then
        return true
    end
    return false
...
function Queue:GetServerShuttingDown()
    if _Queue.isServerShuttingDown == 1
    then
        return true
    end
    return false
...
function Queue:IsPriority(ids, ignoreTemp)
    local prio = false
    ...
    if ignoreTemp then return prio and prio or false end
...
function Queue:GetIds(src)
    ...
    if Config.RequireSteam then
        local steamid = Queue:IsSteamRunning(src)
        ...
        if steamid then
            ...
            for _, id in ipairs(ids) do
                if id ~= steamid and Queue:IsPriority({id}) and id ~= steamid then _ids[#_ids+1]= id end
            end
...
function Queue:IsConnectingFull()
    if not _Queue.MaxConnecting then return false end
    return Queue:ConnectingSize() >= _Queue.MaxConnecting
end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    if GetPlayerEndpoint(src) == "127.0.0.1" then joinDelay = 0 end
    ...
    local function done(msg, _deferrals)
        ...
        if not msg then
            deferrals.done()
            if Config.EnableGrace then Queue:AddPriority(ids[1], Queue:IsPriority(ids, true) + 2, Config.GraceTime) end
        else
    ...
    if Queue:NotFull(true) and joinDelay <= GetGameTimer() and not Queue:IsConnectingFull() then
        -- let them in the server
        local added = Queue:AddToConnecting(ids, true, true, done)
        if not added then CancelEvent() return end
    ...
    while true do
        ...
        if pos <= 1 and Queue:NotFull() and joinDelay <= GetGameTimer() and not Queue:IsConnectingFull() then
            -- let them in the server
...
AddEventHandler("playerDropped", function()
    ...
    if Queue:GetPlayerList()[src] then
        ...
        Queue:RemoveFromQueue(ids)
        Queue:RemoveFromConnecting(ids)
        if Config.EnableGrace then Queue:AddPriority(ids[1], Queue:IsPriority(ids, true) + 5, Config.GraceTime) end
    end
end)
...
commands.setactive = function(args)
    if not args[1] then return end
    _Queue.isServerActive = tonumber(args[1])
end

commands.setshuttingdown = function(args)
    if not args[1] then return end
    _Queue.isServerShuttingDown = tonumber(args[1])
end
...
SetHttpHandler(function(req, res)
    if req.method == "GET" then
        if req.path == "/queuecount" then
            res.send(json.encode({count = Queue:GetSize(),players = Queue:GetPlayerCount()}))
        else
            res.send(json.encode{error = "Bad request"})
        end
    else
        res.send(json.encode({error = "Bad method"}))
    end
end)